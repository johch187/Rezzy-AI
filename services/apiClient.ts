...
const validateApiBaseUrl = (url: string): boolean => {
  ...
  try {
    ...
    // Check if it has a valid hostname (not just 'api' without a domain)
    const hostname = urlObj.hostname;
    // Allow localhost, IP addresses, and domains with at least one dot (or localhost)
...
export const postJson = async <TResponse>(path: string, body: any): Promise<TResponse> => {
  ...
  if (!validateApiBaseUrl(API_BASE_URL)) {
    throw new Error(`VITE_API_BASE_URL is invalid: "${API_BASE_URL}". It must be a valid URL with a proper domain (e.g., http://localhost:8000 or https://api.example.com).`);
  }

...
const updateSummaryFunctionDeclaration: FunctionDeclaration = {
  ...
  description:
    "Updates the user's professional summary in their main profile. Use this whenever the user asks to write, rewrite, improve, or change their summary.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      newSummary: {
...
const navigateToResumeGeneratorDeclaration: FunctionDeclaration = {
  ...
  description:
    "Navigates the user to the resume and cover letter generation page. Use this tool when the user wants to create application documents for a specific job.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      jobDescription: {
        type: Type.STRING,
        description:
          'The full job description for the role the user wants to apply for. The AI should ask the user for this if not already provided in the chat.',
      },
    },
...
const navigateToCoffeeChatDeclaration: FunctionDeclaration = {
  ...
  description:
    'Navigates the user to the Coffee Chat helper page. Use this when the user wants help preparing for a networking chat or wants to write an outreach message.',
  parameters: {
    type: Type.OBJECT,
    properties: {
      counterpartInfo: {
        type: Type.STRING,
        description:
          "All available information about the person the user wants to connect with (e.g., name, title, company, bio, LinkedIn profile text).",
      },
      mode: {
        type: Type.STRING,
        enum: ['prep', 'reach_out'],
...
const promptToCreateCareerPathDeclaration: FunctionDeclaration = {
  ...
  description:
    "Displays a special UI prompt to the user asking for their permission to generate a new career path. Call this function INSTEAD of asking the user with text in the chat.",
  parameters: {
    type: Type.OBJECT,
    properties: {
      currentRole: {
        type: Type.STRING,
        description:
          "The user's current professional situation (e.g., 'University Student', 'Junior Software Engineer'). Infer this from the conversation or the user's profile.",
      },
      targetRole: {
        type: Type.STRING,
        description:
          "The career goal or job title the user is aiming for (e.g., 'Investment Banker', 'Senior Product Manager').",
      },
      isReplacing: {
        type: Type.BOOLEAN,
        description:
          'Set to true if the user already has a career path and this would replace it. This allows the UI to show the correct confirmation message.',
      },
    },
...
const startMockInterviewDeclaration: FunctionDeclaration = {
  ...
  description:
    'Initiates a text-based mock interview session within the chat. Call this when the user wants to practice for an interview.',
  parameters: {
    type: Type.OBJECT,
    properties: {
...
const quantifyImpactDeclaration: FunctionDeclaration = {
  name: 'quantifyImpact',
  description:
    "Analyzes a user's project or work experience description and suggests relevant metrics to quantify their impact.",
  parameters: {
    type: Type.OBJECT,
    properties: {
...
const reframeFeedbackDeclaration: FunctionDeclaration = {
  ...
  description:
    'Helps the user reframe constructive or negative feedback into actionable opportunities for growth.',
  parameters: {
    type: Type.OBJECT,
    properties: {
...
const getNegotiationPrepDeclaration: FunctionDeclaration = {
  ...
  description:
    'Provides salary negotiation preparation, including average salary data and specific talking points based on the role and location.',
  parameters: {
    type: Type.OBJECT,
    properties: {
...
const buildSystemInstruction = (profile: ProfileData, documentHistory: DocumentGeneration[]): string => `
...
export const handleCareerCoachMessage = async (req: AuthedRequest, res: Response) => {
  try {
    const { profile, documentHistory, messages } = req.body as {
      profile?: ProfileData;
    ...
    const functionCalls = candidates.flatMap(candidate =>

...
const MILESTONE_VIDEO_FALLBACK: YouTubeVideo[] = [
  {
...
export const generateCareerPath = async (
  ...
  const parsed = JSON.parse(jsonText) as CareerPath;
  return { ...parsed, currentRole, targetRole };
};

export const getVideosForMilestone = async (
  targetRole: string,
...
export const findMentorMatch = async (
  thesisTopic: string,
  facultyList: string
): Promise<MentorMatch[]> => {
...
export const getNegotiationPrep = async (jobTitle: string, location: string): Promise<{
  salaryRange: string;
  tips: string;
}> => {

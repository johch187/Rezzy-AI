...
const sanitizeWorkspace = (payload) => ({
...
export const handleGetWorkspace = async (req, res) => {
    ...
    if (!data) {
        ...
        return res.json(sanitizeWorkspace({
            profile: defaultProfile,
            documentHistory: [],
            careerChatHistory: [],
            tokens: DEFAULT_TOKEN_BALANCE,
...
export const handleUpsertWorkspace = async (req, res) => {
    const userId = req.user?.id;
    if (!userId) {
        return res.status(401).json({ error: 'Unauthorized' });
    }
    const payload = sanitizeWorkspace(req.body);

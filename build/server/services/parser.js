...
const transformApiResponseToProfile = (parsedData) => {
    const transformSimpleArray = (arr) => {
        if (!arr)
            return [];
        return arr.map(name => ({ id: crypto.randomUUID(), name }));
    };
    const transformExperience = (exps) => {
        if (!exps)
            return [];
        return exps.map(exp => ({
            ...
            endDate: exp.endDate || '',
            achievements: (exp.achievements || []).map((text) => ({ id: crypto.randomUUID(), text })),
        }));
    };
    const transformArrayWithId = (arr) => {
        if (!arr)
            return [];
        return arr.map(item => ({
            id: crypto.randomUUID(),
...
export const parseResumeText = async (resumeText, model) => {
...
export const parseCoverLetterMarkdown = async (coverLetterMarkdown) => {

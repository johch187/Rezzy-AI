export const profileToMarkdown = (data, order) => {
    if (!data)
        return '';
    let md = '';
    if (data.fullName)
                ...
                if (data.experience && data.experience.length > 0) {
                    ...
                    data.experience.forEach(exp => {
                        ...
                        if (exp.startDate || exp.endDate)
                            sectionMd += `*${exp.startDate} - ${exp.endDate}*\n\n`;
                ...
                if (data.education && data.education.length > 0) {
                    ...
                    data.education.forEach(edu => {
                        ...
                        if (edu.startDate || edu.endDate)
                            sectionMd += `*${edu.startDate} - ${edu.endDate}*\n\n`;
                    });
                }
                break;
            case 'skills': {
            ...
            case 'projects':
                if (data.projects && data.projects.length > 0) {
                    ...
                    data.projects.forEach(proj => {
                        sectionMd += `**${proj.name}**\n`;
                        if (proj.description)
                            sectionMd += `${proj.description}\n`;
                        if (proj.technologiesUsed)
                            sectionMd += `*Technologies: ${proj.technologiesUsed}*\n`;
                        sectionMd += '\n';
                    });
                }
                break;
